...
SPARK_JOB_EXECUTOR_IMAGE=${SPARK_JOB_EXECUTOR_IMAGE:=master.mesos.watersupply.labs.platformprovider.com:5001/platformprovider/spark-krb-dispatcher-support:2.1.0}
...
METADATA_INTANCE=${METADATA_INTANCE:=broker-0-confluent-kafka-sec}
...
METADATA_CONF_DIR=${METADATA_CONF_DIR:=/etc/sds/}${METADATA_INTANCE}
...
METADATA_LOG_DIR=/var/log/sds/${METADATA_INTANCE}
...
VAULT_TOKEN=${VAULT_TOKEN:=d4799429-5e49-fe8c-0093-51bd8d947b1d}
# CONFIRMAR CON EL EQUIPO DE PRODUCTO EL CAMBIO QUE HE HECHO
# REFERENCIA DE LOS VALORES CAMBIADOS EN
# https://platformprovider.atlassian.net/wiki/display/TIR/WaterSupply+Environment
# VHOSTS=${VHOSTS:=localhost}
VHOSTS=${VHOSTS:=vault.service.watersupply.labs.platformprovider.com}
...
function main() {
  ...
  KAFKA_KEYSTORE_PASSWORD=br0k3r_0_c0nflu3nt_k4fk4_s3c
  ...
  mv -f ${APPLICATION_JAR_DIR}app.jar .
  ...
  echo "104.199.80.135  vault.service.watersupply.labs.platformprovider.com" >> /etc/hosts
  cat /etc/hosts
  ls -lrht /

  ls -lrth /etc/sds
  cat /etc/sds/broker-0-confluent-kafka-sec/application.properties

  # java -Djava.security.auth.login.config=/etc/sds/broker-0-confluent-kafka-sec/application.properties -Dserver.port=$PORT0 -jar /app.jar

  # JAVA_OPTS="-Xms1024m -Xmx2048m"
  JAVA_OPTS="-Xms1024m -Xmx1024m -XX:PermSize=1024m -XX:MaxPermSize=2048m -XX:+UseConcMarkSweepGC -XX:+CMSClassUnloadingEnabled -Dsun.rmi.dgc.client.gcInterval=36000 -Dsun.rmi.dgc.server.gcInterval=36000"

  env

  java $JAVA_OPTS  -jar /app.jar
  		# --spring.cloud.stream.kafka.binder.configuration.ssl.truststore.location=/kafka-server-trustore.jks
  		# --spring.cloud.stream.kafka.binder.configuration.security.protocol=SSL

  	    # -Dssl.keystore.location=/client_kafka.jks \
  		# -Dssl.keystore.password=br0k3r_0_c0nflu3nt_k4fk4_s3c \
  		# -Dssl.truststore.location=/kafka-server-trustore.jks \
  		# -Dssl.truststore.password=c4_trust_d3f4ult_k3yst0r3 \
  		# -Djavax.net.ssl.trustStore=/client_kafka.jks \
  		# -Djavax.net.ssl.trustStorePassword=c4_trust_d3f4ult_k3yst0r3 \
  		# --spring.cloud.stream.kafka.binder.configuration.ssl.keystore.location=/client_kafka.jks \
		# --spring.cloud.stream.kafka.binder.configuration.ssl.keystore.password=br0k3r_0_c0nflu3nt_k4fk4_s3c \
  		# --spring.cloud.stream.kafka.binder.configuration.ssl.truststore.location=/kafka-server-trustore.jks \
		# --spring.cloud.stream.kafka.binder.configuration.ssl.truststore.password=c4_trust_d3f4ult_k3yst0r3
